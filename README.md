...
```bash
git clone https://github.com/ok9ppp/Zelio.git
cd Zelio
```
...
3. **三级风险** (风险权重 = 10)
   ...
   - **判断要点**: 只要有可能危及生命或造成重大损伤即可认定为极重风险
   - **例子**: 重大医疗事故、严重药物毒性反应或不可逆的器官损伤

#### （三）风险评分与分级

- **计算公式**: 风险评分 = ∑(风险发生率 × 风险权重)
- **评级标准**:
  - **高风险**: 评分 > 5
  - **中风险**: 3 < 评分 < 5
  - **低风险**: 评分 < 3
  - **注**: 手术后出血和疤痕都至少为中风险，出血必然需要干预且发生率100%

### 三、时间数据评估

1. **治疗总时长**: 包括诊断、治疗、养护、后期监测及注意事项
2. **单次治疗时间**: 治疗一次所需要花费的时间
3. **治疗频率**: 需要进行治疗的频次

### 四、治疗费用评估

1. **医保覆盖**: 是否进医保，如果进医保报销比例是多少
2. **付费模式**: 是否采用疗效付费模式
...
2. **时间成本** (治疗频次 × 单次时间):
   - 单月小于7小时: 5分 (例:每日服药三次，每次5分钟，一个月需要花7小时服药)
   - 单月7-14小时: 4分
   - 单月14-21小时: 3分
   - 单月21-28小时: 2分
   - 单月28-35小时: 1分
...
系统综合利用多种数据来源和AI技术进行分析：

1. **疗效、治疗时间**: 使用飞书进行论文分析与总结
2. **风险评估**: 使用DeepSeek进行总结，由人类医生进行审核
3. **费用估算**: 使用大模型回答，数据来源全网
4. **便利度评估**: 由大模型依据评分算法进行判断
